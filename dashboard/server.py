...
class DashboardServer:
    ...
    def _ensure_ssl_certs() -> bool:
        ...
        key_path = certs / "mark.key"
        crt_path = certs / "mark.crt"
        ...
        try:
            ...
            subject = issuer = x509.Name([
                x509.NameAttribute(NameOID.COMMON_NAME, u"Mark LIII Dashboard"),
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"Mark LIII Local"),
            ])
            ip = _local_ip()
            san_items = [
            ...
            print(f"[Dashboard] Auto-generated SSL certificate: {crt_path}")
            return True
        except Exception as e:
            print(f"[Dashboard] Auto SSL generation failed: {e}")
            return False

    @classmethod
    def _ssl_enabled(cls) -> bool:
        cls._ensure_ssl_certs()
        certs = BASE_DIR / "config" / "certs"
        return (
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in Mark to get a new QR code.</p>
</div></body></html>""")
...
<p>Connecting to Mark…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in Mark UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
