...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_mark,
# system_status).
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Mark, an intelligent personal AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Mark checks these topics once a day and alerts the user when there is a new development. "
...
class JarvisLive:
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[MARK] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "Mark").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "Mark"
            _user_name = ""
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name in ("shutdown_mark", "shutdown_jarvis"):
                self._shutdown_requested = True
                self.ui.write_log("SYS: Shutdown requested.")
                async def _do_shutdown():
                    try:
                        await asyncio.wait_for(self._save_session_summary(), timeout=3.0)
                    except Exception:
                        pass
                    if self.session:
                        try:
    ...
    async def _listen_audio(self):
                ...
                try:
                    ...
                    if level >= 0.88:
                        ...
                        if self._shout_frames == 5:
                            self.ui.set_emotion("ANGRY")
                    else:
                        self._shout_frames = max(0, self._shout_frames - 1)
                except Exception:
                    pass
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[MARK] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[MARK] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[MARK] 🔗 Session resumption armed")
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[MARK] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes MARK if asleep.
                    if self._wake_enabled and not self._awake:
    ...
    async def run(self):
        ...
        # Gate callback wiring: reversible actions notify the user; session-log
        ...
        while not getattr(self, "_shutdown_requested", False):
            try:
                ...
                if getattr(self, "_shutdown_requested", False) or "cannot schedule new futures after shutdown" in str(e):
                    print("[MARK] Shutdown requested or process terminating. Exiting cleanly.")
                    break
                ...
                if _is_reconnect_signal(e):
                    print("[MARK] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[MARK] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[MARK] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
            ...
            print(f"[MARK] Reconnecting in {delay}s...")
