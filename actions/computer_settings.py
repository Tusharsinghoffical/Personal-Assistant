...
def volume_get() -> int | None:
    ...
    try:
        ...
        print(f"[Settings] volume_get error: {e}")
        return None
...
_ALIASES = {
    "volume_up":       ("louder", "raise volume", "turn it up", "increase volume", "volume badhao", "volume badha do", "awaaz badhao", "awaaz tez karo", "tez karo", "volume up"),
    "volume_down":     ("quieter", "lower volume", "turn it down", "decrease volume", "volume kam karo", "volume ghatao", "awaaz kam karo", "awaaz dheere karo", "dheere karo", "volume down"),
    "mute":            ("silence", "sound off", "no sound", "mute kardo", "awaaz band karo", "mute kar do"),
    "unmute":          ("unmute", "unmute kardo", "awaaz chalu karo"),
    "brightness_up":   ("brighter", "raise brightness", "increase brightness", "brightness badhao"),
    "brightness_down": ("dimmer", "dim", "lower brightness", "decrease brightness", "brightness kam karo"),
    ...
    "screenshot":      ("capture screen", "take a screenshot", "snip", "screenshot lo", "screenshot le lo", "screen capture"),
    "show_screenshot": ("show screenshot", "screenshot dikhao", "jo screenshot liya tha wo dikhao"),
    "take_photo":      ("take photo", "capture photo", "meri photo lo", "photo lo", "photo kheencho"),
    "show_photo":      ("show photo", "photo dikhao", "meri photo dikhao", "jo photo li hai wo dikhao"),
...
_VALUE_ACTIONS = {"volume_set", "type_text", "press_key", "reload_n",
                  "scroll_up", "scroll_down", "screenshot", "take_photo", "show_photo", "show_screenshot"}
...
def _detect_action(description: str) -> dict:
    ...
    # 2. "set volume to 30", "volume 50 kardo", "awaaz 80%" — a number next to a volume word.
    num = re.search(r"(\d{1,3})\s*%?", low)
    if num and any(w in low for w in ("volume", "ses", "sound", "lautstark", "громкость", "awaaz", "awaz", "bol", "audio")):
    ...
    if action in ("screenshot", "take_screenshot"):
        try:
            ...
            if sc_path and sc_path.exists():
                ...
                return f"Screenshot taken and saved to: {sc_path}."
        except Exception as e:
            print(f"[Settings] Screenshot error: {e}")
        take_screenshot()
        return "Screenshot captured."

    if action in ("show_screenshot", "view_screenshot"):
        from actions.camera_photo import camera_photo_action
        return camera_photo_action({"action": "show_screenshot"}, player=player)

    if action in ("take_photo", "photo", "capture_photo"):
        from actions.camera_photo import camera_photo_action
        return camera_photo_action({"action": "take_photo"}, player=player)

    if action in ("show_photo", "view_photo"):
        from actions.camera_photo import camera_photo_action
        return camera_photo_action({"action": "show_photo"}, player=player)
