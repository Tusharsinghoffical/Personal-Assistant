...
def _capture_camera() -> tuple[bytes, str]:
    if _CV2:
        try:
            ...
            print(f"[Vision] cv2 camera capture failed: {e}")

    # Fallback to Qt camera capture
    try:
        from actions.camera_photo import capture_photo
        p, b = capture_photo()
        if b:
            return b, "image/jpeg"
    except Exception as e:
        print(f"[Vision] Qt camera capture fallback failed: {e}")
